...
type McpKeyTemplateConstraintEntity = {
  id: string;
  key_id: string;
...
type ProjectSecretTemplateBindingEntity = {
  id: string;
  secret_id: string;
  template_id: string;
  created_at: Date;
...
class FakePersistence implements Persistence {
  ...
  public async createAgentTemplate(input: {
    ...
    const template: AgentTemplateEntity = {
      ...
      output_schema_json: input.output_schema ?? null,
      is_enabled: true,
      created_at: now,
      updated_at: now
    };
...
describe("smoke-core API", () => {
  ...
  it("creates, rotates and revokes project secrets without returning raw value", async () => {
    ...
    expect(listResponse.json().items[0].profile_bindings).toEqual([]);
  ...
  it("manages project secret role/profile bindings", async () => {
    const profile = await persistence.createAgentProfile({
      name: "secret-profile",
      role: "reviewer",
      source_policy: "catalog_only",
    ...
    const createResponse = await app.inject({
      ...
      payload: {
        ...
        bind_profile_ids: [profile.id],
        bind_roles: ["Reviewer"]
      }
    });
    expect(createResponse.statusCode).toBe(201);
    const secretId = createResponse.json().id as string;
    expect(createResponse.json().profile_bindings).toHaveLength(1);
    ...
    const removeProfileResponse = await app.inject({
      method: "DELETE",
      url: `/api/projects/project/secrets/${secretId}/bindings/profiles/${profile.id}`,
      headers: { "x-admin-token": config.adminToken }
    });
    expect(removeProfileResponse.statusCode).toBe(200);
    expect(removeProfileResponse.json().ok).toBe(true);
  });
  ...
  it("auto-dispatch applies configured sandbox and approval policy overrides", async () => {
    ...
    const capturingExecutor: DelegationExecutor = {
      execute: async (input) => {
        capturedTemplate = input.target_profile;
        return {
  ...
  it("returns 404 for removed agent templates endpoints", async () => {
    const createResponse = await app.inject({
      method: "POST",
    ...
    const patchResponse = await app.inject({
      method: "PATCH",
      url: "/api/agents/templates/template-1",
      headers: { "x-admin-token": config.adminToken },
      payload: {
  ...
  it("applies MCP profile constraints in authz evaluate", async () => {
    const createProfileResponse = await app.inject({
      method: "POST",
      url: "/api/agent-profiles",
      headers: { "x-admin-token": config.adminToken },
    ...
    const bindConstraintResponse = await app.inject({
      method: "POST",
      url: `/api/mcp/keys/${keyId}/constraints/profiles/${profileId}`,
      headers: { "x-admin-token": config.adminToken }
    });
    ...
    const forbiddenTemplateResponse = await app.inject({
      ...
      payload: {
        api_key: secret,
        tool_name: "orchestrator.list_tasks",
        agent_profile_id: "another-profile"
      }
    });
