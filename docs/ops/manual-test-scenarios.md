...
8. В `История` задай фильтры `search/status/profile` и проверь, что список событий фильтруется.
9. В `Лимиты` раскрой карточку аккаунта и проверь детали 5h/week/reset.
