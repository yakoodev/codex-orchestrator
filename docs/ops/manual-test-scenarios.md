...
4. Проверь, что карточка показывает:
   ...
   - `prompt` (в раскрывающемся блоке);
   - `account` (label + status);
   - `log` preview (в раскрывающемся блоке).
...
2. В блоке `Auth Profiles` проверь, что профили показываются карточками (не таблицей).
3. Нажми `Activate` у неактивного профиля.
4. Проверь, что у карточки появляется метка `активный сейчас`.
5. Нажми `Deactivate` и проверь, что метка активного профиля пропадает.
