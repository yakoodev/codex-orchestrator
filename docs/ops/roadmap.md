...
- Текущий прогресс:
  ...
  - задачи переведены из табличного вида в task-tracker board (колонки: `ожидает запуска` / `запущена` / `выполнена`);
  - в задачах добавлены client-side фильтры (`search/project/status`) и карточка `Task Details` по выбору задачи;
  - форма создания задачи переведена в collapsible-режим и автосворачивается после первого успешного создания;
  - `Held Queue` и списки колонок переведены в фиксированные scroll-зоны для плотного операционного экрана;
  - в UI добавлен контекст текущей вкладки (операционный intro-блок с явным сценарием по разделу);
  - `Auth Profiles` переведены из таблицы в карточки с быстрыми `activate/deactivate` действиями;
  - `Accounts & Limits` разделены на внутренние подтабы `Profiles/Limits/History`;
  - `Switch Events` получили фильтры (`search/status/profile`) и компактный detail-on-demand просмотр;
  - добавлены panel states (`loading/success/error`) и автообновление (toggle, по умолчанию off) для `Tasks`, `Agents`, `Switch History`;
  - ручная правка памяти агентов убрана из основного агентского потока в скрытый fallback-блок в `System`.
- Остается:
  - визуальная полировка плотности экранов (типографика, контраст, баланс отступов);
  - дополнительная эргономика mobile-pass (360/768) на реальных сценариях;
  - точечный accessibility pass (aria/focus keyboard flow) после стабилизации верстки.
- Новые задачи по операторскому фидбеку (2026-04-08):
  - [x] сделать задачи в формате task-tracker доски с 3 колонками по жизненному циклу:
    - `ожидает запуска`,
    - `запущена`,
    - `выполнена`;
  - [x] ручное управление памятью агентов спрятать как fallback/admin-override (не держать как основной сценарий на виду);
  - [~] улучшить навигацию, логичность IA и общий UI/UX (вкладочный intro и карточки профилей уже внедрены; остаются compact action panels + mobile UX-pass).
