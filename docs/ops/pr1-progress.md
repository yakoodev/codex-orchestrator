...
- [x] Добавлен best-effort механизм съема лимитов для нескольких `CODEX_HOME`: `npm run limits:check` (sqlite `codex.rate_limits` snapshot + latest usage-limit hit + optional live probe `codex exec`).
