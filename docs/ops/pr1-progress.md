...
- [x] По операторскому фидбеку убраны внутренние скроллы у карточек `Agents` (скролл только страницей), добавлен перенос длинных `agent id` без горизонтального overflow.
