...
const I18N = {
  ru: {
    ...
    route_system: "System",
    route_logs: "Logs",
    route_desc_dashboard: "Health и последние сигналы системы.",
    route_desc_tasks: "Создание, фильтрация и управление очередью задач.",
    route_desc_agents: "Мониторинг preparing/running/recent агентов и инспектор.",
    route_desc_accounts: "Профили auth.json, лимиты и история переключений.",
    route_desc_memory: "Память агентов по project_id и role.",
    route_desc_system: "Конфигурация модуля и история исполнений.",
    route_desc_logs: "Централизованный журнал UI-операций и API-ответов.",
    language_label: "Язык",
    theme_label: "Тема",
    theme_dark: "Темная",
    theme_light: "Светлая",
    ...
    no_preparing_agents: "Нет preparing-агентов.",
    no_running_agents: "Нет running-агентов.",
    no_recent_agents: "Нет recent-агентов.",
...
function renderLogs() {
  ...
  ui.logsList.innerHTML = filtered.slice(0, 200).map((item) => {
    ...
    return `<li class="log-item"><div class="log-head"><span class="log-level ${levelClass(item.level)}">${escapeHtml(item.level)}</span><span class="pill">${escapeHtml(item.scope)}</span><span class="meta-note">${escapeHtml(fmtDate(item.ts))}</span></div><div>${escapeHtml(item.message)}</div>${details ? `<pre class="mono-box">${escapeHtml(details)}</pre>` : ""}</li>`
  }).join("")
}
