...
const I18N = window.__CODEX_ORCH_I18N__ ?? { ru: {}, en: {} }
...
function syncProjectBindings() {
  ...
  const profilePlaceholder = profileOptions.length ? t("task_field_na") : t("executor_no_runnable_profiles")
  const taskProfileId = applyGenericSelect(
    ui.taskAgentProfile,
    profileOptions,
    ui.taskAgentProfile?.value ?? "",
    profilePlaceholder
  )
  const scheduleProfileId = applyGenericSelect(
  ...
  if (taskSubmit) taskSubmit.disabled = !taskProject || !taskProfileId
  if (memorySubmit) memorySubmit.disabled = !memoryProject
  if (scheduleSubmit) {
    scheduleSubmit.disabled = !scheduleProject || !scheduleProfileId
  }
...
function renderAgents(payload) {
  ...
  const renderBucket = (node, list, emptyKey) => {
    ...
    node.innerHTML = list.slice(0, 30).map((item) => {
      const profileMeta = item.target_profile ?? null
      const template = profileMeta ? `${profileMeta.name} (${profileMeta.model})` : t("task_field_na")
      ...
      return `<li class="agent-card ${state.selectedAgentId === item.id ? "agent-card-selected" : ""}" data-agent-id="${escapeHtml(item.id)}">
        ...
        <div class="agent-card-meta">${escapeHtml(t("agent_field_profile"))}: ${escapeHtml(template)}</div>
  ...
  const selectedProfileMeta = selected.target_profile ?? null
  const template = selectedProfileMeta ? `${selectedProfileMeta.name} (${selectedProfileMeta.model})` : t("task_field_na")
  ...
  ui.agentInspectorProfile.textContent = template
...
function renderSecrets(items) {
  ...
  ui.secretsList.innerHTML = visible.map((secret) => {
    const roles = Array.isArray(secret.role_bindings)
      ...
      : []
    const templates = Array.isArray(secret.profile_bindings)
      ? secret.profile_bindings.map((item) => item?.profile_id).filter((value) => typeof value === "string" && value.trim())
      : []
    ...
    return `<li class="secret-card" data-secret-id="${escapeHtml(secret.id)}" data-secret-project="${escapeHtml(secret.project_id)}" data-secret-active="${String(active)}">
      ...
      <div class="secret-bindings">
        ...
        <div class="meta-note">${escapeHtml(t("secrets_bindings_profiles"))}: ${templates.length ? `<code>${escapeHtml(templates.join(", "))}</code>` : escapeHtml(t("task_field_na"))}</div>
      </div>
      <div class="secret-actions">
        ...
        <button type="button" class="button-ghost" data-secret-action="bind-profile">${escapeHtml(t("secrets_action_bind_profile"))}</button>
        <button type="button" class="button-ghost" data-secret-action="unbind-profile">${escapeHtml(t("secrets_action_unbind_profile"))}</button>
...
function wireConsoleHandlers() {
  ...
  ui.secretCreateForm.addEventListener("submit", async (event) => {
    ...
    const bindTemplates = parseCsvList(ui.secretBindProfiles.value)
    const payload = {
      ...
      bind_profile_ids: bindTemplates
    }
  ...
  ui.secretsList.addEventListener("click", async (event) => {
    ...
    if (action === "bind-profile" || action === "unbind-profile") {
      const profileId = prompt(t("secrets_prompt_profile"))?.trim()
      if (!profileId) return
      const method = action === "bind-profile" ? "POST" : "DELETE"
      await requestJson(
        `/api/projects/${encodeURIComponent(projectKey)}/secrets/${encodeURIComponent(secretId)}/bindings/profiles/${encodeURIComponent(profileId)}`,
        { method }
      )
      pushLog("success", "ui", t("log_secret_binding_updated"), {
        project_id: projectKey,
