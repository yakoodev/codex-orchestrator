...
import { IMPLEMENTED_ROUTES } from "./http/implemented-routes";
import {
  AGENT_PROFILE_SCRIPT_OSES,
  AGENT_PROFILE_SCRIPT_TYPES,
  AGENT_PROFILE_SOURCE_POLICIES,
  AGENT_REQUEST_RESOLVE_ALLOWED_STATUSES,
  AGENT_REQUEST_STATUSES,
  AGENT_REQUEST_TERMINAL_STATUSES,
  AGENT_REQUEST_TYPES,
  AUTH_CONTEXT_TYPES,
  DEFAULT_AUTH_SWITCH_RETRY_LIMIT,
  DEFAULT_DELEGATION_TIMEOUT_RETRY_LIMIT,
  DEFAULT_PROJECT_SUMMARY_WINDOW_HOURS,
  DELEGATION_APPROVAL_POLICIES,
  DELEGATION_SANDBOX_POLICIES,
  MAX_AGENT_PROFILE_PROMPT_MCP_SERVERS,
  MAX_AGENT_PROFILE_PROMPT_SCRIPT_LENGTH,
  MAX_AGENT_PROFILE_SCRIPT_CONTENT_LENGTH,
  MAX_DELEGATION_EXECUTION_PROMPT_LENGTH,
  MAX_DELEGATION_LOG_LENGTH,
  MAX_DELEGATION_PROMPT_LENGTH,
  MAX_MCP_KEY_ACL_TOOLS,
  MAX_MCP_KEY_PROFILE_BINDINGS,
  MAX_MEMORY_CONTENT_LENGTH,
  MAX_MEMORY_CONTEXT_ENTRIES,
  MAX_PROJECT_SECRET_BINDINGS,
  MAX_PROJECT_SECRET_VALUE_LENGTH,
  MAX_PROJECT_SUMMARY_WINDOW_HOURS,
  MAX_RUNTIME_SECRET_ENV_VARS,
  MCP_API_KEY_PATCH_STATUSES,
  MCP_API_KEY_STATUSES,
  MCP_SERVER_ORIGIN_TYPES,
  MCP_SERVER_TRANSPORTS,
  ORCHESTRATOR_MCP_SERVER_NAME,
  PROJECT_KEY_PATTERN,
  PROJECT_SECRET_KEY_PATTERN,
  SCHEDULE_MISFIRE_POLICIES,
  SCHEDULE_OVERLAP_POLICIES,
  SCHEDULE_SCOPES,
  TASK_AUTODISPATCH_FAILURE_TEXT_PATTERNS,
  TASK_AUTODISPATCH_PICKUP_STATUSES,
  TASK_AUTODISPATCH_RESULT_FAILED_MARKER,
  TASK_AUTODISPATCH_RESULT_SUCCESS_MARKER,
  TASK_AUTODISPATCH_RUNNING_STATUSES,
  TASK_AUTODISPATCH_SOURCE,
  TASK_AUTODISPATCH_WAITING_LIMIT_ERROR_CODES
} from "./http/app-constants";
import type {
  AgentProfileCreateRequest,
  AgentProfileMcpBindingRequest,
  AgentProfilePatchRequest,
  AgentProfileScriptUpsertRequest,
  AgentRequestCreateRequest,
  AgentRequestResolveRequest,
  AuthContextCreateRequest,
  AuthContextPatchRequest,
  DelegationDispatchRequest,
  McpApiKeyCreateRequest,
  McpApiKeyPatchRequest,
  McpAuthzEvaluateRequest,
  ...
  ProjectPatchRequest,
  ProjectSecretCreateRequest,
  ProjectSecretPatchRequest,
  ProjectSecretRotateRequest,
...
import {
  agentProfileMcpBindingToResponse,
  agentProfileScriptSetToResponse,
  agentProfileToResponse,
  agentRequestAuditEventToResponse,
  agentRequestToResponse,
  artifactToResponse,
  authContextToResponse,
  authProfileToResponse,
  buildDelegationCardsResponse,
  buildMcpApiKeyResponse,
  buildProjectSecretResponse,
  delegationToResponse,
  getAgentRequestStatusEventType,
  mcpKeyProfileBindingToResponse,
  mcpKeyProfileConstraintToResponse,
  mcpServerRegistryToResponse,
  moduleExecutionToResponse,
  packToResponse,
  projectSummaryToResponse,
  projectSecretProfileBindingToResponse,
  projectSecretRoleBindingToResponse,
  projectToResponse,
  scheduledRuleToResponse,
  scheduledRunToResponse,
  taskToResponse,
  toAuthProfileDisplayId,
  workerToResponse
} from "./http/response-serializers";
import {
  asFiniteNumber,
  asNonNegativeInteger,
  evaluateScheduleRuleAst,
  parseScheduleEvaluationContext,
  runScheduleRecoveryOnStartup
} from "./http/schedule-evaluator";
import {
...
function createDefaultDelegationExecutor(): DelegationExecutor {
  return {
    async execute(input) {
      const targetId = input.target_profile?.id ?? "unknown-profile";
...
export async function createApp(deps: AppDependencies): Promise<FastifyInstance> {
      ...
      if (
        ...
        const failedRun =
          (await persistence.updateScheduledRun(startedRun.id, {
            status: "failed",
            ended_at: new Date(),
            skip_reason: "task_profile_incomplete",
            result_json: {
              matched: true,
              reasons,
              trigger_source: options.triggerSource
        ...
        await publisher.publish({
          eventType: "schedule.run.failed",
          traceId: options.traceId,
          idempotencyKey: `${failedRun.id}:task_profile_incomplete:${options.traceId}`,
          payload: {
            rule_id: options.rule.id,
            run_id: failedRun.id,
            scope: options.rule.scope,
            status: failedRun.status,
            reason: "task_profile_incomplete"
          }
        });
        return failedRun;
      }
  ...
  app.post("/api/projects/:key/secrets", async (request, reply) => {
    ...
    try {
      profileIds = parseOptionalStringArray(body.bind_profile_ids, "bind_profile_ids", {
    ...
    if (profileIds.length > 0) {
      const profiles = await persistence.listAgentProfiles({ include_disabled: true, limit: 500 });
      const availableProfileIds = new Set(profiles.map((item) => item.id));
      const missingProfileId = profileIds.find((profileId) => !availableProfileIds.has(profileId));
      if (missingProfileId) {
        return sendError(
          reply,
          404,
          `Agent profile not found: ${missingProfileId}`,
    ...
    for (const profileId of profileIds) {
      await persistence.bindProjectSecretToProfile(createdSecret.id, profileId, "admin");
    }
    ...
    await writeSecretAuditEventBestEffort(
      persistence,
      {
        ...
        metadata_json: {
          ...
          profile_bindings_count: profileIds.length,
  ...
  app.post("/api/projects/:key/secrets/:id/bindings/profiles/:profileId", async (request, reply) => {
    const { key: keyParam, id: secretId, profileId } = request.params as {
      key: string;
      id: string;
      profileId: string;
    };
    const projectKey = asProjectKey(keyParam);
    if (!projectKey || !asNonEmptyString(secretId) || !asNonEmptyString(profileId)) {
      return sendError(reply, 400, "Invalid project key, secret id or profile id", "VALIDATION_ERROR");
    }
    ...
    const binding = await persistence.bindProjectSecretToProfile(secret.id, profileId, "admin");
    if (!binding) {
      return sendError(reply, 404, "Secret or profile not found", "NOT_FOUND");
    }
    ...
    return reply.send(projectSecretProfileBindingToResponse(binding));
  });

  app.delete(
    "/api/projects/:key/secrets/:id/bindings/profiles/:profileId",
    async (request, reply) => {
      const { key: keyParam, id: secretId, profileId } = request.params as {
        key: string;
        id: string;
      ...
      if (!projectKey || !asNonEmptyString(secretId) || !asNonEmptyString(profileId)) {
        return sendError(
          reply,
          400,
          "Invalid project key, secret id or profile id",
          "VALIDATION_ERROR"
        );
      }
      ...
      const deleted = await persistence.unbindProjectSecretFromProfile(secret.id, profileId);
      if (!deleted) {
        return sendError(reply, 404, "Secret profile binding not found", "NOT_FOUND");
      }
  ...
  app.post("/api/mcp/keys/:id/constraints/profiles/:profileId", async (request, reply) => {
    const { id, profileId } = request.params as { id: string; profileId: string };
    ...
    if (!profile) {
      return sendError(reply, 404, "Agent profile not found", "NOT_FOUND");
    }

    const constraint = await persistence.bindMcpKeyProfileConstraint(id, profileId, "admin");
    if (!constraint) {
      return sendError(
        reply,
        404,
    ...
    return reply.send(mcpKeyProfileConstraintToResponse(constraint));
  });

  app.delete("/api/mcp/keys/:id/constraints/profiles/:profileId", async (request, reply) => {
    const { id, profileId } = request.params as { id: string; profileId: string };
    ...
    const deleted = await persistence.unbindMcpKeyProfileConstraint(id, profileId);
    if (!deleted) {
      return sendError(reply, 404, "MCP key profile constraint not found", "NOT_FOUND");
    }
  ...
  app.post("/api/mcp/authz/evaluate", async (request, reply) => {
    ...
    const profileConstraints = await persistence.listMcpKeyProfileConstraints(key.id);
    if (profileConstraints.length > 0) {
      ...
      const isProfileAllowedByConstraint = profileConstraints.some(
        (constraint) => constraint.agent_profile_id === agentProfileId
      );
